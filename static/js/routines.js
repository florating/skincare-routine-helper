...
function create_routine_steps(am_or_pm, routine) {
	...
	for (const [ categoryId, categoryName ] of Object.entries(routine)) {
      ...
      <li class="ui-state-default step"
        data-routine-type="${am_or_pm}"
        data-category-id="${categoryId}"
        name="${categoryId}"
        id="${am_or_pm}_routine-category_id_${categoryId}"
      >
        ${categoryName}:
        <select class="category_id_${categoryId}"
          data-category-id="${categoryId}"
          name="product_id"
        >
...
function generate_complete_routine(am_or_pm, routine) {
...
$(() => {
	...
	$('#create-am-routine').on('click', () => {
		...
		$.post('/get_cabinet', '', (res) => {
			...
			let routineLv = document.querySelector('input[name="routine-level"]:checked').value;
			generate_complete_routine('am', routineLv);
	...
	$('#create-pm-routine').on('click', () => {
		...
		$.post('/get_cabinet', '', (res) => {
			...
			// PM ROUTINE:
			let routineLv = document.querySelector('input[name="routine-level"]:checked').value;
			generate_complete_routine('pm', routineLv);
